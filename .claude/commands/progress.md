...
4. **工时留痕（必做）**：完成实际工作（文书/研究/整理）后执行 `log-work`：内容=工作摘要、`--file`=产出文书、`--task`=关联任务（自动）；时长用户提及则用，未知传 `?` 待律师补录。示例：`/progress 251112 起诉状改完了花了2小时` → set-status + log-work 2 "修改起诉状" --task task_xxx --file "06 - 📝 法律文书/xxx.docx"
