...
4. **工时留痕（必做）**：完成实际工作（文书/研究/整理）后执行 `log-work`：内容=工作摘要、`--file`=产出文书、`--task`=关联任务（自动）。时长三态：用户说了实际耗时（"花了2小时"）→ 用实际；没说 → `auto`（按类型下限外扩：文书/研究 1h、核查/沟通/整理 0.5h——计入人工核查的合理冗余）；用户明确不想估 → `?` 待补。示例：`/progress 251112 起诉状改完了` → set-status + `log-work 251112 auto "修改起诉状" --task task_xxx --file "06 - 📝 法律文书/xxx.docx"`
5. **校验汇报**：写入后 `validate` 确认，输出变更摘要（before → after）。
