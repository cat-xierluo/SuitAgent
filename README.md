...
- **使用Zed（推荐）**：在左侧文件树中展开案件文件夹（根目录）
- **使用终端**：运行 `open [案件编号]/` (macOS) 或 `start [案件编号]\` (Windows)
